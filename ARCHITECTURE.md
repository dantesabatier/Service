...
The ReAct loop exposes one synthetic tool the executor never sees: **`run_subagent`** — launch a nested ReAct run on the same catalogue to complete one bounded task. The child environment retains the same client, executor, execution policy and context assembler, so remote or isolated execution remains in force, every in-process data tool still enforces its own RBAC, shared budgets cannot be reset by delegating and context limits apply identically; the recursion guard is structural (`canSpawnSubagents: false`), not a prompt. It inherits the parent's system prompt exactly — the model may supply task context, never replacement policy. The subagent's final answer is returned as the tool result, and its tokens are charged to the parent run.
