...
A custom tool must declare what each call must be authorized for by implementing `authorizationRequirements(Dictionary $arguments): ?AuthorizationRequirements`. `ToolRegistry` checks the declaration against the request security context before `execute()` runs; with security enabled, a tool that declares nothing, or whose declaration comes out empty, is denied. Return `AuthorizationRequirements::none()` for a call that touches no entity, or every `(resource, action)` the call reaches — `readRequirements()`, `predicateKeyPaths()`, `shapeKeyPaths()` and `writeRequirements()` compute them the way the built-in tools do. With no security context in effect, as in a command-line job, every call is denied — unless the application's access policy is not `DefaultAccessPolicy`, in which case it has no security to enforce.

A custom tool must also apply the same row- and field-level helpers the built-in tools use — the MCP request URL is always `/mcp`, so none of the URL-driven guards that protect a regular endpoint apply, and a tool that skips them reads or writes rows the caller is not entitled to:
...
- **`enforceEntityAuthorization($resource, $action)`** — only for a permission that cannot be known before `execute()` runs; anything that can belongs in the declaration.
...
Remote descriptions and JSON Schema are not authorization metadata. `MCPToolExecutor` therefore treats every remote call as state-changing and non-cacheable unless the application supplies trusted `readOnly` and `cacheable` closures that classify the concrete `LLMToolCall`; the secure default keeps write approval in force even when a remote description claims it only reads. The catalogue handshake has its own bounded bootstrap timeout because `LLMToolExecutor::$tools` is resolved before a model turn. Every execution receives the root run's live `LLMExecutionDeadline`, and the executor passes its remaining time to `tools/call`. The Streamable HTTP transport never retries a call: losing its response does not prove a remote write did not happen. A transport or protocol failure becomes `LLMRunStopReason::toolProviderFailure`, retaining whether a later run is worth attempting, while a remote `isError` result remains a correctable tool message the model can act on. The low-level `MCPClientException` deliberately stays outside the `InternalInconsistencyException` funnel that `ToolRegistry` turns into model feedback; the executor normalizes it into `LLMToolProviderException`, whose `InvalidRequestException` base preserves the framework's structured HTTP 500 response if a custom loop lets it escape. A `404` clears the expired client session so a later run reinitializes it.

```php
$transport = new StreamableHTTPMCPTransport($endpoint, new Dictionary([
    "Authorization" => "Bearer $token",
]));
$executor = new MCPToolExecutor(
    new MCPClient($transport),
    readOnly: fn(LLMToolCall $call): bool => in_array($call->name, ["search", "describe_model"], true),
    cacheable: fn(LLMToolCall $call): bool => $call->name === "describe_model",
);
$agent = new LLMAgent($modelClient, $executor, timeLimit: 30.0);
```

The in-process adapter remains deliberately synchronous: it checks the deadline before and after dispatch but cannot preempt arbitrary PHP code, and adds no sandbox boundary of its own. Moving execution behind HTTP isolates the agent process and gives its wait a hard transport timeout, but the remote server's own CPU, memory, filesystem and cancellation boundaries still depend on how that server is deployed.

`LLMAgent` is the stable configuration facade rather than the orchestration algorithm. It adapts a `ToolRegistry`, resolves the default policies and services, clones the caller's conversation, and wraps the request in an `LLMAgentRuntime` before invoking an `LLMAgentLoop`. The default `ReActLLMAgentLoop` preserves the original model → tools → results cycle; `PlanExecuteLLMAgentLoop` requires a valid ordered plan before a real tool may execute, while still allowing a task that needs no tool to answer directly. A planning or domain-specific implementation can replace either without replacing its provider, executor, context assembler, observer or clock. A strategy depends on the narrow `LLMAgentSession` contract rather than the concrete runtime and receives no raw `LLMClient`, `LLMToolExecutor`, execution state, deadline or observer. It returns only an `LLMAgentLoopOutcome`; the runtime constructs `LLMRun` exclusively from the messages and tokens it recorded, and rejects a claimed completion without a conclusive assistant turn. Changing the decision algorithm therefore retains the framework's iteration and token budgets, write approval, run-local cache, shared deadline and structured span lifecycle.

The ReAct and Plan–Execute strategies ask their `LLMAgentSession` for provider turns and feed guarded execution results back until the model finishes. The runtime owns the accumulated history and usage; counts each provider turn before dispatch; assembles context; applies approval, shared budgets and the run-local cache; and appends every assistant and tool message it produced. Terminal guardrail state is irreversible: even if a strategy catches the internal interruption used to unwind its stack, the runtime retains the original deadline, budget or provider ending and uses it instead of the strategy's returned outcome. Refusals and output limits are normalized inside the runtime and can never be presented as completion. Every requested call remains pending until the runtime records its exact result; a strategy cannot substitute a fabricated invocation or open another provider turn while results are missing. Real tools use `executeTool`; strategy-owned descriptors are advertised separately. A strategy may complete one of its own synthetic calls, reject any pending call to enforce its current phase, or route a guarded child run through the subagent capability. Every path verifies the exact model invocation, consumes the shared tool budget, appends its result and closes its trace span, so a loop cannot relabel a real write to bypass approval. One monotonic `LLMExecutionDeadline` is created for the root and passed unchanged to provider calls, tools and child runtimes, so delegation cannot reset the window. `LLMClient` bounds each transport attempt to the remaining time at the transport's one-second granularity and refuses a retry whose delay no longer fits; a response that arrives too late is discarded and normalized as `LLMRunStopReason::deadline`. `LLMToolCallParser` similarly normalizes the action envelope: Standard supplies arguments as JSON text, while Anthropic and Ollama supply decoded objects. Every call must have a usable identifier and name and an argument object; malformed JSON and lists are provider failures rather than silently becoming an empty argument dictionary. This boundary deliberately does not reinterpret or validate domain values, which remain the tool's responsibility. Before each turn, an `LLMContextAssembler` receives the complete history, system prompt and advertised tools. The default `WindowedLLMContextAssembler` may bound message count or a custom-measured context size, drops only complete conversation turns, pins system messages and the newest turn, and can insert an application-supplied summary of omitted history. A provider-specific tokenizer plugs in as the measurement closure; a different context or memory strategy replaces the assembler without changing the loop or provider client. If the protected context alone cannot fit, the run stops with `contextLimit` rather than silently discarding policy or half a tool exchange.

`Sabatier\Service\Testing\LLMAgentLoopConformanceTestCase` is the executable contract for third-party loops. Its optional PHPUnit-backed namespace ships with the package without loading during application execution. A concrete test supplies only an `LLMAgentLoop`; the shared cases exercise it through an actual runtime and verify direct completion, guarded execution, an irreversible iteration cap and write approval. Both built-in loops run through that same suite, so replaceability is continuously demonstrated rather than inferred from interface shape alone.

`RetrievalAugmentedLLMContextAssembler` is the backend-neutral long-term-memory/RAG layer. It asks an `LLMRetriever` for ranked `LLMRetrievedDocument` values using the latest non-empty user query plus a disposable copy of the conversation history. Retrieval occurs on each context assembly because assistant and tool evidence may have changed even when the latest user text has not; backend-specific freshness and caching therefore remain the retriever's responsibility. The framework does not prescribe embeddings, a vector database or even vector search: textual, hybrid, SQL, remote and vector implementations share the same contract. By default at most eight documents and 32 KiB of formatted retrieval context are considered. Empty or oversized documents are skipped without truncating their contents, and provenance fields remain beside each document.

Retrieved text is inserted as a separate user message immediately before the latest user turn, never interpolated into the system prompt. The system prompt receives only a fixed framework rule that marks those delimited documents as untrusted evidence rather than instructions. Each document boundary carries an unguessable token drawn per assembly, and the prefix tells the model that only boundaries bearing that token are genuine: without it the delimiters are derived from a document index a hostile document can count to, so text spelling out its own closing boundary would end its block early and continue outside the frame the rule declares as data. The content itself is never escaped — it is evidence the model is meant to read — while provenance fields are flattened to one line each, since a newline inside a `Key: value` line would forge the others. The wrapped assembler still applies the final context window and may drop the complete retrieval message before the user's own turn; when it does, the decorator reassembles the original context without the now-unnecessary retrieval rule. Retrieval failures propagate by default instead of silently producing an answer without knowledge the application may require. An optional-memory retriever can express degradation explicitly by returning an empty collection.

`LLMExecutionPolicy` adds coercive limits for tool calls, subagent launches and provider-reported token usage; its counters are shared by the complete run tree. It also denies state-changing calls by default. An application that has obtained user approval supplies a closure that approves only the concrete `LLMToolCall` values covered by it, while `AbstractTool::isReadOnlyCall()` lets a mixed tool classify the operation selected by its arguments.

`LLMRunObserver` watches the runtime while it runs through one `observe(LLMRunEvent)` entry point. `LLMRun` reports what a finished run cost and why it stopped, which answers the question afterwards; the event stream answers it as the run proceeds and reports what the return value cannot express at all — which iteration assembled context or called a tool, how long each boundary took, whether the run's cache answered instead of the tool, and which subagent did the work. The runtime owns those boundaries: from outside, a run is one blocking call, which is why an application cannot stream progress without this hook.

The framework emits immutable start and terminal events for runs, context assembly, model turns and tool calls. `LLMContextAssemblyFinishedEvent` reports retained and omitted message counts, compaction, limit status and total assembly duration — including retrieval — without exposing prompt or document contents. An assembler exception produces `LLMContextAssemblyFailedEvent` before the run failure; a context that simply cannot fit produces a normal finished assembly span and stops the run before a model-turn span opens. Retriever-specific candidate counts, embedding latency and cache statistics remain the concrete retriever's responsibility. Terminal tool events classify execution, cache hits, denials, exhausted budgets, deadline expiry, remote-provider failure and program failure explicitly; a provider deadline closes its model span as failed while the run itself closes normally with the `deadline` stop reason. An unavailable remote tool provider similarly closes the open tool span and finishes the run with `toolProviderFailure`; it is not reported as an uncaught application fault. Unexpected model and run exceptions still close their failure spans. Events contain scalar snapshots rather than live `LLMContext`, `LLMTurn` or `LLMRun` collections, and they deliberately omit prompt text, tool arguments and tool results. Every event carries an `LLMRunContext`; a subagent reports under its own identifier while naming its parent, so nested work stays attributable. Durations and deadlines use an injectable monotonic `LLMClock`, while event timestamps use Unix time.

The framework defines the event vocabulary; persisting, logging, counting or streaming it is the application's responsibility. The observer is optional and inherited by every subagent. Delivery is synchronous and best-effort by default: an observer failure is logged without changing the agent result. `LLMRunObserverFailurePolicy::strict` is available when trace delivery is part of the caller's correctness boundary and should abort the run instead.

The ReAct loop exposes one synthetic tool the executor never sees: **`run_subagent`** — launch a nested ReAct run on the same catalogue to complete one bounded task. `LLMAgentSession::executeSubagent` is the only launch path exposed to a strategy: it consumes the shared tool and subagent budgets, derives the child environment, uses `LLMExecutionPolicy::$maxSubagentIterations`, and structurally disables further delegation. A child retains the same client, executor, execution policy, execution state, deadline, context assembler and observer, so remote or isolated execution remains in force, every in-process tool call is still authorized by the registry, shared budgets and time cannot be reset by delegating, and context limits apply identically. It inherits the parent's system prompt exactly — the model may supply task context, never replacement policy. The subagent's final answer is returned as the tool result, and its tokens are charged to the parent run.
