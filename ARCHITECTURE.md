...
`LLMRunObserver` watches the loop while it runs through one `observe(LLMRunEvent)` entry point. `LLMRun` reports what a finished run cost and why it stopped, which answers the question afterwards; the event stream answers it as the run proceeds and reports what the return value cannot express at all — which iteration a tool call belonged to, how long it took, whether the run's cache answered it instead of the tool, and which subagent did the work. Only the loop sees those boundaries: from outside, a run is one blocking call, which is why an application cannot stream progress without this hook.

The framework emits immutable start and terminal events for runs, model turns and tool calls. Terminal tool events classify execution, cache hits, denials, exhausted budgets and failures explicitly; model and run failures also close their spans when an unexpected exception escapes. Events contain scalar snapshots rather than live `LLMContext`, `LLMTurn` or `LLMRun` collections, and they deliberately omit prompt text, tool arguments and tool results. Every event carries an `LLMRunContext`; a subagent reports under its own identifier while naming its parent, so nested work stays attributable. Durations and deadlines use an injectable monotonic `LLMClock`, while event timestamps use Unix time.
