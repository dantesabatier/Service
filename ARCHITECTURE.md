...
`LLMRunObserver` watches the loop while it runs. `LLMRun` reports what a finished run cost and why it stopped, which answers the question afterwards; the observer answers it as the run proceeds and reports what the return value cannot express at all — which iteration a tool call belonged to, how long it took, whether the run's cache answered it instead of the tool, and which subagent did the work. Only the loop sees those boundaries: from outside, a run is one blocking call, which is why an application cannot stream progress without this hook. Each callback carries an `LLMRunContext` identifying the run, and a subagent reports under its own identifier while naming its parent, so nested work stays attributable. The framework decides what the events are; persisting, logging, counting or streaming them is the application's. The observer is optional and inherited by every subagent, and the loop calls it synchronously without guarding against it throwing.
