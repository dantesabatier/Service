...
Retrieved text is inserted as a separate user message immediately before the latest user turn, never interpolated into the system prompt. The system prompt receives only a fixed framework rule that marks those delimited documents as untrusted evidence rather than instructions. The wrapped assembler still applies the final context window and may drop the complete retrieval message before the user's own turn; when it does, the decorator reassembles the original context without the now-unnecessary retrieval rule. Retrieval failures propagate by default instead of silently producing an answer without knowledge the application may require. An optional-memory retriever can express degradation explicitly by returning an empty collection.
