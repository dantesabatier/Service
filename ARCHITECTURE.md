...
`LLMAgent` is the stable configuration facade rather than the orchestration algorithm. It adapts a `ToolRegistry`, resolves the default policies and services, clones the caller's conversation, and delegates an `LLMAgentRunRequest` plus an immutable `LLMAgentEnvironment` to an `LLMAgentLoop`. The default `ReActLLMAgentLoop` preserves the original model → tools → results cycle; a planning or domain-specific implementation can replace it without replacing the provider, executor, context assembler, observer or clock. A custom loop is trusted application code and owns orchestration: because the environment exposes those services directly, it must apply the supplied execution policy and emit the trace boundaries it claims to support. Injecting a loop is therefore an explicit replacement of the control algorithm, not a way to bypass the default ReAct loop while retaining its guarantees automatically.

The ReAct strategy loops over `LLMClient` turns and feeds execution results back until the model finishes or the iteration budget runs out. Approval, shared budgets and the run-local cache stay in that loop because they decide that no execution occurs at all; the executor never has to reproduce agent policy. One monotonic `LLMExecutionDeadline` is created for the root and passed unchanged to provider calls, tools and subagents, so delegation cannot reset the window. `LLMClient` bounds each transport attempt to the remaining time at the transport's one-second granularity and refuses a retry whose delay no longer fits; a response that arrives too late is discarded and normalized as `LLMRunStopReason::deadline`. `LLMTurnStopReason` normalizes the provider's ending before the loop acts on it, so a refusal or output limit remains an incomplete run rather than looking complete merely because it contains no tool calls. `LLMToolCallParser` similarly normalizes the action envelope: Standard supplies arguments as JSON text, while Anthropic and Ollama supply decoded objects. Every call must have a usable identifier and name and an argument object; malformed JSON and lists are provider failures rather than silently becoming an empty argument dictionary. This boundary deliberately does not reinterpret or validate domain values, which remain the tool's responsibility. Before each turn, an `LLMContextAssembler` receives the complete history, system prompt and advertised tools. The default `WindowedLLMContextAssembler` may bound message count or a custom-measured context size, drops only complete conversation turns, pins system messages and the newest turn, and can insert an application-supplied summary of omitted history. A provider-specific tokenizer plugs in as the measurement closure; a different context or memory strategy replaces the assembler without changing the loop or provider client. If the protected context alone cannot fit, the run stops with `contextLimit` rather than silently discarding policy or half a tool exchange.
...
`LLMRunObserver` watches the loop while it runs through one `observe(LLMRunEvent)` entry point. `LLMRun` reports what a finished run cost and why it stopped, which answers the question afterwards; the event stream answers it as the run proceeds and reports what the return value cannot express at all — which iteration assembled context or called a tool, how long each boundary took, whether the run's cache answered instead of the tool, and which subagent did the work. Only the loop sees those boundaries: from outside, a run is one blocking call, which is why an application cannot stream progress without this hook.
...
The ReAct loop exposes one synthetic tool the executor never sees: **`run_subagent`** — launch a nested ReAct run on the same catalogue to complete one bounded task. The child environment retains the same client, executor, execution policy, execution deadline and context assembler, so remote or isolated execution remains in force, every in-process data tool still enforces its own RBAC, shared budgets and time cannot be reset by delegating, and context limits apply identically; the recursion guard is structural (`canSpawnSubagents: false`), not a prompt. It inherits the parent's system prompt exactly — the model may supply task context, never replacement policy. The subagent's final answer is returned as the tool result, and its tokens are charged to the parent run.
