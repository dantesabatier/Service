...
The JSON-RPC server is one face of the tool catalogue. `LLMAgent` drives tools through an `LLMToolExecutor`: the executor supplies the descriptors advertised to the model, classifies concrete calls for approval and caching, and executes calls that passed the loop's policy checks. Passing a `ToolRegistry` to the agent remains the convenient in-process path; it is adapted automatically through `InProcessLLMToolExecutor`. An application may instead inject a process-isolated or remote executor without changing the loop or provider client. Every execution receives the root run's live `LLMExecutionDeadline`, so an isolated executor can turn the remaining time into a process or RPC timeout. The in-process adapter is deliberately synchronous: it checks the deadline before and after dispatch but cannot preempt arbitrary PHP code, and adds no sandbox boundary of its own. Hard interruption and isolation therefore require an executor whose underlying mechanism can enforce them.
