...
#### Request security context and the write observer

`Application` fixes a `RequestSecurityContext` — the first responder's effective policy, the subject, its scopes and the managed object context — once access is enforced, and keeps it through the send. A responder that declares its content public (`isProtectedContentAvailable = true`) sets `isContentPublic`, which exempts only its own writes from the observer (`enforcesWrites` is false), since no authorization admitted it. `ToolRegistry` runs every tool call under `enforcingWrites()`, so a tool's writes are checked even there. It does **not** turn off `isSecurityEnabled`: a public responder that runs tools still has every call authorized, so it cannot hand them unrestricted access. Do not collapse the two flags back into one. Both `ToolRegistry` and `WriteAuthorizationObserver` read this one context, so they cannot reach different decisions; `AbstractTool` derives its security state from it too.

`WriteAuthorizationObserver` runs on `ManagedObjectContextWillSave` for that context's managed object context only, and holds every inserted, updated and deleted object to `create`/`update`/`delete` on its entity, ownership and resource-level `#[Writable]` on the committed values, under `own` the owner after the change too, field-level `#[Writable]` on every changed property, and `read` plus row visibility for every existing row a write links. The to-many side of a link made from the to-one side is exempt; nothing else is. It stays out of the way with no context (a CLI job), with security disabled, with no authenticated subject, and inside `RequestSecurityContext::performAsSystem()` — which is for the system's own bookkeeping (logout's token bump, token invalidation), never for data the request set out to change.

With no context, the observer does nothing but the registry **denies** every tool call. The asymmetry is deliberate: a CLI job writes as the system, but an agent started from it must not inherit unrestricted access by accident — the job fixes a context with `RequestSecurityContext::perform()`. Do not make the registry pass without a context — with one exception: when `Application::$accessPolicy` is not `DefaultAccessPolicy`, the application chose to have no security at all, and tools run without a context as they would inside a request. `Application::$accessPolicy` stays the switch for the whole stack.

#### MCP tools and row-level rules

An MCP request URL is always `/mcp`, so none of the URL-driven guards protecting a regular endpoint apply; `mcp:create` only admits the caller to the channel. Entity-level RBAC is enforced by `ToolRegistry` before `execute()`, against `AbstractTool::authorizationRequirements()`: undeclared (the default `null`) or empty is denied under security, `AuthorizationRequirements::none()` declares no entity, otherwise every `(resource, action)` the call reaches — projection, predicate, sort, grouping and nested values, not only the entity the tool is named after. Derive it from the same private functions `execute()` uses, without changing `execute()`'s signature. Row- and field-level rules stay in the tool, and a tool that skips them reads or writes rows the caller is not entitled to:
