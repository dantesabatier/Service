 ...
 *
 * This abstract class provides a unified structure for metadata-driven security. Beyond role membership (`$by`) and ownership scope (`$scope`), a property may carry an attribute-based condition (`$where`) expressed as a predicate format string evaluated against the resource, the authenticated subject (`$SUBJECT`), and the request environment (`$ENVIRONMENT`).
 */
abstract readonly class FieldAttribute
{
     ...
     * @param string|null $where Predicate format string gating access to this property, evaluated against the resource with `$SUBJECT` and `$ENVIRONMENT` substitution variables. Null means no attribute-based condition.
