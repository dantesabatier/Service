...
final readonly class ToolResolver
{
    ...
    private function discover(): ArrayClass
    {
        ...
        return $this->fileURLs($directoryURL)->map(fn(URL $url): string => "App\\" . MCPToolsDirectory . "\\" . FileManager::default()->displayName($url->path))->filter($this->isValidToolClass(...))->map(fn(string $class): AbstractTool => new $class($this->context, $this->descriptor));
    }
