...
final class ReadPersistentSpaceResponseStrategy extends PersistentSpaceResponseStrategy
{
    ...
    private function executeSecureFetch(FetchRequest $fetchRequest): Response
    {
        ...
        $secureTransform = fn(ManagedObject $object): Dictionary => $this->applySecureRead($object, $object->jsonSerialize());
        if ($fetchRequest->fetchBatchSize) {
            return new StreamResponse($this->request->url, $fetchRequestResult, chunkSize: $fetchRequest->fetchBatchSize, transform: $secureTransform);
        }
